...
func parseForm(r *http.Request) (height, width int, color string) {
	...
	reqHeight, isThere := r.Form["height"]
	if isThere {
		if numHeight, err := strconv.Atoi(reqHeight[0]); err != nil {
			log.Printf("Invalid height: %q", reqHeight[0])
		} else {
			height = numHeight
		}
	}
	reqWidth, isThere := r.Form["width"]
	if isThere {
		if num_width, err := strconv.Atoi(reqWidth[0]); err != nil {
			log.Printf("Invalid width: %q", reqWidth[0])
		} else {
			width = num_width
		}
	}
